...
import { useEditAssignmentMutation } from "../../../features/admin/assignment/assignmentAPI";
import { useGetAdminVideosQuery } from "../../../features/admin/videos/videosAPI";
import TextInput from "../form/TextInput";

function EditAssignment({ open, control, assignment: a }) {
  const { data: videos, isError } = useGetAdminVideosQuery();
  const [editAssignment, { isLoading }] = useEditAssignmentMutation();
  const [title, setTitle] = useState(a.title);
  const [video_title, setVideoTitle] = useState(a.video_title);
  const [video_id, setVideoId] = useState(a.video_id);
  const [totalMark, setTotalMark] = useState(a.totalMark);
  ...
  const handleForm = (e) => {
    ...
    const confirm = window.confirm("Confirm Edit Assignment");
    if (confirm) {
      editAssignment({ data: formData, id: a.id });
      reset();
      control();
    }
  };
