...
import {
  addQuizOption,
  getSingleQuiz,
} from "../../../features/admin/quizzes/quizzesSlice";
...
function QuizItem({ quiz }) {
  ...
  const handleEdit = () => {
    dispatch(getSingleQuiz(quiz));
    dispatch(addQuizOption(quiz?.options));
  };

  return (
    <tr>
      <td className="table-td">{question}</td>
      <td className="table-td"> {truncateString(video_title, 30)} </td>
      <td className="table-td flex gap-x-2">
        ...
        {/* edit button  */}
        <Link to="/admin/edit-quiz" onClick={handleEdit}>
          <svg
            fill="none"
            viewBox="0 0 24 24"
            strokeWidth="1.5"
